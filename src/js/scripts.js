...
window.addEventListener('load', async event => {
    ...
    updateNews().then().catch(error => {
        alert(`Errors in getting news: ${error}`);
    });

    await getSources().then().catch(error => {
    ...
    selectSource.addEventListener('change', e => {
        ...
        updateNews(e.target.value).then().catch( error => {
...
async function getSources() {
    ...
    selectSource.innerHTML = json.sources.map(src => `<option value="${src.id}">${src.name}</option>`).join('\n');
}

async function updateNews(source = defaultSource) {
    let topic = 'apple';

    const res = await fetch(`https://newsapi.org/v2/everything?sources=${source}&q=${topic}&from=2019-06-01&to=2019-06-03&sortBy=popularity&apiKey=${apiKey}`);
    const json = await res.json();
...
function createArticles(article) {
    ...
    <div class="article">
        <a href="${article.url}" target="_blank">
            <h2>${article.title}</h2>
            <img src="${article.urlToImage || './src/images/img-not-found.png'}" alt="${article.source.name}">
